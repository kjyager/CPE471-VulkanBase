...
void DualTransferBackedBuffer::initDevice(const VulkanDeviceBundle& aDeviceBundle){
...
void DualTransferBackedBuffer::recordUploadTransferCommand(const VkCommandBuffer& aCmdBuffer) {
...
void DualTransferBackedBuffer::freeStagingBuffer(){
...
void DualTransferBackedBuffer::freeAndReset(){
    freeStagingBuffer();
...
void DualTransferBackedBuffer::createStagingBuffer(VkDeviceSize aRequiredSize, const VkBufferCreateInfo& aBufferInfo, const VmaAllocationCreateInfo& aAllocInfo){
    VmaAllocator allocator = VmaHost::getAllocator(mCurrentDevice);
...
void DualTransferBackedBuffer::createResidentBuffer(VkDeviceSize aRequiredSize, const VkBufferCreateInfo& aBufferInfo, const VmaAllocationCreateInfo& aAllocInfo){
    VmaAllocator allocator = VmaHost::getAllocator(mCurrentDevice);
...
void DualTransferBackedBuffer::prepareBuffersForUploadStaging(size_t aDataSize){
    if(!mCurrentDevice.isValid()){
...
void DualTransferBackedBuffer::stageDataForUpload(const uint8_t* aData, size_t aDataSize){
